...
final myVenuesProvider = StateNotifierProvider<MyVenuesNotifier,
    AsyncValue<List<Map<String, dynamic>>>>((ref) {
  return MyVenuesNotifier(ref.read(businessRepositoryProvider));
});

class MyVenuesNotifier
...
final businessDashboardProvider =
    FutureProvider.family<Map<String, dynamic>, String>((ref, venueId) async {
  final result =
      await ref.read(businessRepositoryProvider).getDashboard(venueId);
  return result.when(
      success: (data) => data, failure: (e) => throw Exception(e));
});

// Auto-refreshing dashboard stream (polls every 30s)
final liveDashboardProvider = StreamProvider.autoDispose
...
final businessAnalyticsProvider = StateNotifierProvider.family<
  ...
  (ref, args) => VenueAnalyticsNotifier(
      ref.read(businessRepositoryProvider), args.venueId, args.period),
);
...
final venueStatusProvider = StateNotifierProvider.family<
    VenueStatusLoadNotifier, AsyncValue<Map<String, dynamic>>, String>(
  (ref, venueId) =>
...
class VenueStatusLoadNotifier
  ...
  Future<void> load() async {
    ...
    final result = await _repository.getVenueStatus(_venueId,
        isWorker: AuthService().currentUser?.role == UserRole.WORKER);
    state = result.when(
      success: AsyncValue.data,
      failure: (e) => AsyncValue.error(e, StackTrace.current),
    );
...
final businessVenueOffersProvider = StateNotifierProvider.family<
...
class BusinessOffersNotifier
    extends StateNotifier<AsyncValue<Map<String, dynamic>>> {
  final BusinessRepository _repository;
  final String _venueId;

  BusinessOffersNotifier(this._repository, this._venueId)
      : super(const AsyncValue.loading()) {
    load();
  }
...
final businessAuthProvider = StateNotifierProvider<BusinessAuthNotifier,
    AsyncValue<Map<String, dynamic>?>>((ref) {
  return BusinessAuthNotifier(ref.read(businessRepositoryProvider));
});

class BusinessAuthNotifier
    extends StateNotifier<AsyncValue<Map<String, dynamic>?>> {
  final BusinessRepository _repository;
  ...
  Future<bool> resetPassword(
    ...
    state = const AsyncValue.loading();
    final result = await _repository.businessResetPassword(
